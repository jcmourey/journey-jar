...
struct TVShowFormView: View {
    @Bindable var store: StoreOf<TVShowForm>
    ...
    var body: some View {
        Form {
            Section("TVShow Info") {
                ...
                KeyContentPair("Pick a team for this show") {
                    TeamSelectorView(store: store.scope(state: \.teamSelector, action: \.teamSelector))
                }
...
#Preview("Add TV Show") {
    NavigationStack {
        let store = Store(initialState: TVShowForm.State(tvShow: .mock.mock1, focus: .title)) {
            TVShowForm()
        }
        TVShowFormView(store: store)
    }
